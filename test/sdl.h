...
class Sdl
{
    ...
    public:
        ...
        * @param V Matrice contenant les coordonnées de la courbe (chaque ligne eprésente un point de la courbe et doit contenir : la coordonnée x et la coordonnée y)
        * @param color Couleur de la courbe, sous la forme d'un vecteur de trois entiers non signés de 8 bits
        */
        void draw_curve(const std::vector<std::vector<double>>& V, const std::vector<Uint8>& color);
